...
import { AlertTriangle, AlertTriangle as ExclamationTriangleIcon, BarChart3 as ChartBarIcon, Briefcase as BriefcaseIcon, Building2, CalendarRange, Coins as BanknotesIcon, FileText as DocumentTextIcon, Landmark, MapPin as MapPinIcon, Percent as PercentIcon, UserRound, Users as UserGroupIcon } from 'lucide-react';
...
export const DashboardHeatMap: React.FC = () => {
  ...
  const stats = [
    { label: 'Total Pembiayaan', value: rupiahRingkas(Math.round(kpi.totalPembiayaan)), icon: BanknotesIcon, bg: 'bg-blue-50 dark:bg-blue-900/20', color: 'text-blue-600 dark:text-blue-400' },
    { label: 'Total Nasabah', value: kpi.totalNasabah.toLocaleString('id-ID'), icon: UserGroupIcon, bg: 'bg-indigo-50 dark:bg-indigo-900/20', color: 'text-indigo-600 dark:text-indigo-400' },
    { label: 'Baki Debet', value: rupiahRingkas(kpi.bakiDebet), icon: DocumentTextIcon, bg: 'bg-cyan-50 dark:bg-cyan-900/20', color: 'text-cyan-600 dark:text-cyan-400' },
    { label: 'Nasabah Bermasalah', value: kpi.bermasalah.toLocaleString('id-ID'), icon: ExclamationTriangleIcon, bg: 'bg-red-50 dark:bg-red-900/20', color: 'text-red-600 dark:text-red-400' },
    ...
    { label: 'Baki Debet Bermasalah', value: rupiahRingkas(kpi.bakiBermasalah), icon: BanknotesIcon, bg: 'bg-rose-50 dark:bg-rose-900/20', color: 'text-rose-600 dark:text-rose-400' },
  ];
  ...
  /*
   * Tanpa data nominatif, dashboard ini tidak ditampilkan sama sekali.
   *
   * Sebelumnya ia tetap tampil penuh memakai angka contoh: Rp 487,3 M total
   * pembiayaan, 4.029 nasabah, NPL 16,0%, lengkap dengan peta kabupaten
   * berwarna dan persentase "dari periode sebelumnya". Semuanya karangan,
   * ditulis tetap di dalam kode. Ada spanduk kuning yang menyebutkannya, tetapi
   * spanduk setinggi satu baris tidak sebanding dengan satu layar penuh angka
   * yang tampak sah — apalagi pada dashboard kepatuhan, yang angkanya dipakai
   * untuk pelaporan OJK dan mudah sekali difoto lalu diteruskan tanpa spanduknya.
   *
   * Ini juga sumber kebingungan yang berulang soal "peta masih per kabupaten":
   * peta per kabupaten itu memang bagian dari data contoh. Lapisan per kelurahan
   * hanya terisi dari data sungguhan, dan data contoh sengaja tidak punya titik
   * desa sama sekali.
   */
  if (memuat) {
    /*
     * Keadaan memuat ditangani di sini, bukan sebagai spanduk di atas dashboard.
     * Selama pengambilan data berjalan, `data` masih berisi nilai contoh; kalau
     * dashboard tetap dirender, angka karangan itu berkedip di layar lebih dulu
     * sebelum diganti angka sungguhan.
     */
    return (
      <div className="w-full h-full p-4 sm:p-6 lg:p-8 overflow-y-auto">
        <div className="mx-auto flex max-w-xl flex-col items-center justify-center py-24 text-center">
          <span className="mb-4 h-8 w-8 animate-spin rounded-full border-2 border-slate-300 border-t-transparent" />
          <p className="text-sm font-bold text-gray-500">Memuat data nominatif kredit…</p>
        </div>
      </div>
    );
  }
      ...
      <div className="w-full h-full p-4 sm:p-6 lg:p-8 overflow-y-auto">
        <div className="mx-auto flex max-w-xl flex-col items-center justify-center py-24 text-center">
          ...
          <h1 className="text-lg font-black tracking-tight text-gray-800">
            Belum ada data nominatif kredit
          </h1>
          <p className="mt-2 text-sm leading-relaxed text-gray-500">
            {catatan ?? 'Belum ada data nominatif kredit yang diunggah.'} Dashboard ini menghitung
            seluruh angkanya dari berkas Nominatif Kredit, jadi tidak ada yang bisa ditampilkan
            sebelum berkasnya ada.
          </p>
          <p className="mt-3 text-xs leading-relaxed text-gray-400">
            Unggah berkas Nominatif Kredit lewat menu Data Center. Agar peta sebaran terisi sampai
            tingkat desa dan bukan hanya per kabupaten, pastikan berkas yang diunggah memuat kolom
            kelurahan.
          </p>
        </div>
      </div>
  ...
  return (
    <div className="w-full h-full p-4 sm:p-6 lg:p-8 overflow-y-auto space-y-6">
      ...
      {/* ---------- Penanda sumber data ----------
          Sampai di sini sumbernya pasti data nominatif sungguhan: keadaan
          memuat dan keadaan tanpa data sudah ditangani lebih dulu di atas. */}
      <div className="flex flex-wrap items-center gap-2 px-4 py-2.5 rounded-xl bg-emerald-50 border border-emerald-200 text-xs text-emerald-800">
        ...
        {stats.map((stat) => {
          return (
            <div key={stat.label} className="bg-white dark:bg-gray-800 p-4 rounded-2xl shadow-sm border border-gray-100 dark:border-gray-700/50 flex flex-col gap-2 min-w-0">
              ...
              {/*
                Tidak ada baris "naik sekian persen dari periode sebelumnya".
                Angka itu dulu berasal dari TREN_PERIODE_SEBELUMNYA, konstanta
                contoh yang ditulis tetap dan tidak pernah dihitung dari apa pun.
                Karena dashboard sekarang hanya tampil saat datanya sungguhan,
                menaruh persentase karangan di sebelah angka sungguhan justru
                membuatnya tampak sebagai hasil perhitungan. Perbandingan antar
                periode akan muncul kembali bila memang sudah dihitung.
              */}
